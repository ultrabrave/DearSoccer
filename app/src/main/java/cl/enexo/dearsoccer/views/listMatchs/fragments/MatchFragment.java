...
public class MatchFragment extends Fragment {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if (conexion == 0)
                {
                    Toast.makeText(getContext(), "No existen partidos para el día de hoy", Toast.LENGTH_SHORT).show();
                }
            }
        },10000);
    }
    ...
    {
        RecyclerView recycler = (RecyclerView) myview.findViewById(R.id.rv_match);
        recycler.setHasFixedSize(true);
        recycler.setLayoutManager(new LinearLayoutManager(getContext()));
        DatabaseReference reference  = FirebaseDatabase.getInstance().getReference().child("match").child("country").child(mycountry).child("city").child(mycity).child("locality").child(mylocality).child("year").child(myyear).child("month").child(mymonth).child("day").child(myday).child("public").child("match");
